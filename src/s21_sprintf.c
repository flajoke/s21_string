...
static void whole_num_to_string(int64_t val, char *ret, int base);
static void unsigned_num_to_string(uint64_t val, char *ret, int base);
static bool check_integer_specifier(char c);
static bool is_all_zeroes(char *buff);
static void to_upper(char *str);
static void prepend_ox(char *buff, flags f);
static void double_to_string(long double val, char *ret, flags f);
static void prepend_mantiss(char *str, int pow, char sign);
static void remove_trailing_zeroes(char *buff);

// parse format
static const char *get_flags(const char *format, flags *f);
static const char *get_width(const char *format, flags *f, va_list va);
static const char *get_precision(const char *format, flags *f, va_list va);
static const char *get_length(const char *format, flags *f);

// parse values
static void handle_value(flags f, char *buff, va_list va);
static void parse_int(flags, char *buff, va_list va);
static void parse_unsigned(flags f, char *buff, va_list va);
static void parse_octal(flags f, char *buff, va_list va);
static void parse_hex(flags f, char *buff, va_list va);
static void parse_char(flags f, char *buff, va_list va);
...
static void parse_mantiss(flags f, char *buff, va_list va);
...
void format_wchar(flags f, char *buff, wchar_t w_c) {
    if (!f.minus && f.width) {
        ...
        for (size_t i = 0; i < f.width - s21_strlen(tmp); i++)
            buff[i] = ' ';
        s21_strcat(buff, tmp);
    } else if (f.width) {
